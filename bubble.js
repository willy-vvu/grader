...
var switchToProblem=function(problemNumber){
	if(currentProblem==null||problemNumber!=currentProblem){
		...
		//Load the code
		$("#submissionArea textarea").val(graders[currentProblem].savedCode);
...
var updateProblemSwitcher=function(){
	problemSwitcher.find("li").each(function(index){
		...
		$(this).css("border-left-color",colorDef[borderColor])
			.removeClass("active");
...
problemSwitcher.parent().mouseenter(function(){
	problemSwitcher.stop().animate({"right":"300px"},500)
})
.mouseleave(function(){
	problemSwitcher.stop().animate({"right":"5px"},500)
})
switchToProblem(0)
//})();